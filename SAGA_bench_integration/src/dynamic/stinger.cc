    ...
    for(stinger_eb* tmp = eb; tmp != nullptr; tmp = (stinger_eb*)readff((int64_t *)&tmp->next)){
        ...
        for(k=0; k < endk; k++){            
            if (dest == (tmp->edges[k].neighbor)){
                ...
                __sync_fetch_and_add(&insFail, 1);
                return;
            }
        }
    }
...
void stinger::search_and_delete_edge(const Edge& e, bool source, stinger_eb* eb, bool in_neighbor)
{
	//delTot++;
    //make stinger edge
    stinger_edge edge(e, source);
    ...
    for(stinger_eb* tmp = eb; tmp != nullptr; tmp = (stinger_eb*)readff((int64_t *)&tmp->next)){
        ...
        for(k=0; k < endk; k++){
            if (dest == (tmp->edges[k].neighbor)){
				...
				if (dest == tmp->edges[k].neighbor) {
	            	//delSucc++;
					writexf(&(eb->edges[k].neighbor), -2);
					stinger_int64_fetch_add(&eb->numEdges, -1);

					if(in_neighbor) in_degree_increment_atomic(eb->node, -1);
					else out_degree_increment_atomic(eb->node, -1);
				}
				writeef(&(tmp->edges[k].weight), weight);
				return;
            }
        }
    }
...
void stinger::updateForVertex(const Edge& e, bool source){
    ...
    if(source || (!source && !directed)){     
    	...
    	if(!e.isDelete){
    		...
    		search_and_delete_edge(e, source, node->out_neighbors, false);
    	}
    }
    	...
    	if(!e.isDelete){
    		...
    		search_and_delete_edge(e, source, node->in_neighbors, true);
    	}
    } 
}
