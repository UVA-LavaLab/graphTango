...
public:
	ld_rhh(int cap, float rehash) :
			rhh<EdgeID, T>(cap, rehash) {
	}
	ld_rhh(int cap) :
			ld_rhh(cap, 0.9) {
	}
...
void ld_rhh<T>::iter::operator++() {
	for (;;) {
		...
		if (pd == parent->get_capacity() || elem.empty() || pd > parent->probe_dist(parent->desired_pos(elem.hash), pos)) {
			cursor = nullptr;
			break;
