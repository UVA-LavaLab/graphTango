...
protected:
	...
	inline uint32_t probe_dist(uint32_t const &origin, uint32_t const &cursor) const;
	inline void do_insert(uint32_t const &pos, uint32_t const &hash, K const &k, V const &v);
...
template<typename K, typename V>
void rhh<K, V>::do_insert(uint32_t const &pos, uint32_t const &hash, K const &key, V const &val) {
	arr[pos].key = key;
	arr[pos].val = val;
	arr[pos].hash = hash;
	++size;
