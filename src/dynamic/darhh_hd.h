...
public:
	hd_rhh(uint32_t ec_cap, float ec_load_factor) :
			ec_cap(ec_cap), load_factor(ec_load_factor) {
	}
	hd_rhh(uint32_t ec_cap) :
			hd_rhh(ec_cap, 0.9) {
	}
	hd_rhh() :
			hd_rhh(pow(2, 5)) {
...
void hd_rhh<T>::iter::operator++() {
	...
	while (ec->arr[pos].empty() || ec->arr[pos].deleted()) {
		if (++pos >= ec->get_capacity()) {
			cursor = nullptr;
			return;
		}
...
typename hd_rhh<T>::iter hd_rhh<T>::begin(NodeID const &id) const {
	...
	if (ret == v_table.end()) {
		...
		if (it.ec->arr[it.pos].empty() || it.ec->arr[it.pos].deleted())
			++it;
		else
